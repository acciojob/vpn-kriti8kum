...
public class ConnectionServiceImpl {
    ...
    public User connect(int userId, String countryName) throws Exception {
        ...
        if (user.getOriginalCountry().getName().equalsIgnoreCase(countryName)) {
            return user;
        }
        ...
        ServiceProvider selectedProvider = serviceProviders.stream()
                .filter(sp -> sp.getCountryList().stream().anyMatch(c -> c.getName().equalsIgnoreCase(countryName)))
    ...
    public User communicate(int senderId, int receiverId) throws Exception {
        ...
        String receiverCurrentCountry = receiver.getConnected() ? receiver.getMaskedIp().split("\\.")[0] : receiver.getOriginalCountry().getName();
        if (sender.getOriginalCountry().getName().equalsIgnoreCase(receiverCurrentCountry)) {
            return sender;
        }
